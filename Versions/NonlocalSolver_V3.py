...
class NonlocalSolver:
    def __init__(self, f: Callable, dL: Callable, t_span: list, y0: np.array, betas: list, alpha: float,
        ...
        self.gamma = lambda t: (1 - self.betas[0]) / (self.alpha * (1 - self.betas[0] ** t)) * np.sqrt((1 - self.betas[1] ** t) / (1 - self.betas[1]))
        self.epsilon = 1e-8

        self.hat_epsilon = lambda t: self.alpha * np.sqrt((1 - self.betas[1] ** t) / (1 - self.betas[1])) * self.epsilon
    ...
    def __rhs_with_integral_part__(self, y: np.array) -> np.array:
        ...
        def integral(t):
            ...
            with ThreadPoolExecutor() as executor:
                future_numerator = executor.submit(fixed_quad, numerador_func, 0, t,n=int(4e2))
                future_denominator = executor.submit(fixed_quad, denominador_func, 0, t, n=int(4e2))
